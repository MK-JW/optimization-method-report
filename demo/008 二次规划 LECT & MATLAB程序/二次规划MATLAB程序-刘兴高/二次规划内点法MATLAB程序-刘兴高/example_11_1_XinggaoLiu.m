function example_11_1_XinggaoLiu
close all;
clear all;
clc;
global x_rec;
...
Aeq=[];
beq=[];
Aineq=[eye(2);-1,-1;-1,2;2,-3];
bineq=[zeros(2,1);-5;-2;-6];
x_initial=[1;3];
[m1,n1]=size(Aeq);
[m2,n2]=size(Aineq);
...
Z=X1.^2+X2.^2-8*X1-6*X2;
contour(X1,X2,Z);
V=[f_optimal,f_optimal];
contour(X1,X2,Z,V);
plot([0,6],[0,0],'k');
plot([0,0],[0,6],'k');
xp=[0,0;2,0;4,1;1.8,3.2;0,2;0,0];
plot(xp(:,1),xp(:,2),'g');
[mm,nn]=size(x_rec);
plot(x_rec(:,1),x_rec(:,2),'-om')
