function example_9_5_XinggaoLiu
close all;
clear all;
clc;
global x_rec
...
min_max=0;
tolerance=1e-4;
[x_optimal,y_optimal,f_optimal,k,lamda_optimal,mu_optimal]=LP_pdi_infeasible_general(c,Aeq,beq,Aineq,bineq,z_initial,min_max,tolerance)
...
function [x_optimal,y_optimal,f_optimal,k,lamda_optimal,mu_optimal]=LP_pdi_infeasible_general(c,Aeq,beq,Aineq,bineq,z_initial,min_max,tolerance)
%=============输入参数======================
% c:     目标函数系数向量
% Aeq：  等式约束系数矩阵
% beq：  等式约束的右端向量
...
while duality_gap>tolerance
    ...
    p_k=-(Aineq')*(vector_temp2+vector_y)+vector_x;
    if isempty(Aeq)
        vector_lamda=[]
    else
        vector_lamda=Aeq*x_k-beq;
    end
    [delta_x,f_QP,exitflag,output,lambda]=quadprog(H,p_k,[],[],Aeq,-vector_lamda,[],[]);
