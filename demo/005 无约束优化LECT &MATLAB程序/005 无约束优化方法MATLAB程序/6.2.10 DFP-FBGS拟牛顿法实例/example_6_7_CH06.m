function example_6_7_CH06
close;
clear;
clc;
%-----------------------------------------------
%刘兴高-应用最优化方法及MATLAB实现CH06-6.7
%例6.7  目标函数f(x)=1+x1+x2+x3+x4+x1*x2+x1*x3+x1*x4+x2*x3+x2*x4+x3*x4+x1^2+x2^2+x3^2+x4^2-0.4*exp(-x5^2-6*x6^2);
%  x0=(-4;0;-4;-1;1;1);  tol=1e-6
%------------------------------------------------
x_initial=[-4;0;-4;-1;1;1];
tolerance=1e-6;
[x_optimal,f_optimal,k]=BFGS_Wolfe(@f_test3,@g_test3,x_initial,tolerance)
end
